...
function prox_admm(model::AbstractBlockNLPModel; options...)
    ...
    prox_aug_blocks = [
        ProxAugmentedNLPBlockModel(
            ...
            b,
            x,
            sparse(opt.proximal_penalty, model.blocks[i].meta.nvar, model.blocks[i].meta.nvar),
        ) for i = 1:nb
    ]
    ...
    while !(converged || tired)
        ...
        max_iter_time = 0.0
        aug_obj_value = 0.0 # reset to zer

        for i = 1:nb
            update_primal!(prox_aug_blocks[i], x)
            update_dual!(prox_aug_blocks[i], y[model.problem_size.con_counter+1:end])
        ...
        opt.update_scheme == :JACOBI && (x = deepcopy(temp_x))
        ...
        converged =
            norm(aug_obj_value - temp_aug_obj_value) / abs(aug_obj_value) <=
